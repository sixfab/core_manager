    ...
    def check_dns(self):
        ...
        if output[2] == 0:
            if output[0].find("nameserver 8.8.8.8") == -1:
                ...
                out = shell_command("sudo echo '# Generated by Sixfab Network Manager' > /etc/resolv.conf")
                out1 = shell_command("sudo echo 'nameserver 8.8.8.8' >> /etc/resolv.conf")
                out2 = shell_command("sudo echo 'nameserver 8.8.4.4' >> /etc/resolv.conf")
