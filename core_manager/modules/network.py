...
class Network():
    ...
    def adjust_metric(self, interface, metric):
        ...
        try :
            ...
            raise RuntimeError('Error occured setting metrics', error)
        else:
            return 0
    ...
    def check_dns(self):
        ...
        """
        output = shell_command("cat /etc/resolv.conf")

        if output[2] == 0:
            if output[0].find("nameserver 8.8.8.8") == -1:
                # Add default DNS server
                out = shell_command('echo "# Generated by Sixfab CORE" | sudo tee -a /etc/resolv.conf')
                out1 = shell_command('echo "nameserver 8.8.8.8" | sudo tee -a /etc/resolv.conf')
                out2 = shell_command('echo "nameserver 8.8.4.4" | sudo tee -a /etc/resolv.conf')

                if out1[2] != 0 or out2[2] != 0:
                    raise RuntimeError('Error occured adding default DNS servers!')
            else:
                pass
